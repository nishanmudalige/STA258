...
used to compare multiple nears
Suppose we have $k$ groups $(k \geqslant 3)$ and each group is given a treatment.

The A NOVA model is
![](https://cdn.mathpix.com/cropped/2025_07_03_0e611eebd5aab7c4aee1g-02.jpg?height=679&width=1752&top_left_y=609&top_left_x=21)
equimient form of ANOLA is

$$
Y_{i j}=\mu_{i}+\varepsilon_{i j}, \quad \varepsilon_{i j} \sim N\left(0, \sigma^{2}\right)
$$
...
|  | $x_{12}$ | $x_{22}$ |  | $x_{k 2}$ |
|  | $\vdots$ |  |  | $\vdots$ |
| sample <br> mean <br> sample <br> size <br> size | $x_{1 n}$ | $x_{2 n}$ |  | $x_{k n k}$ |
|  | $x_{1}$ | $x_{1}$ | $\bar{x}_{2}$ |  |
|  |  |  | $\bar{x}_{k}$ |  |
|  | $n_{2}$ |  | $S_{k}^{2}$ |  |
|  |  |  | $n_{k}$ |  |

total sample size $n=n_{1}+n_{2}+\ldots+n_{k}$ over all sample mean $\bar{x}=\frac{1}{n} \sum_{i=1}^{k} \sum_{j=1}^{n_{i}} x_{i j}=\frac{1}{n} \sum_{i=1}^{k} n_{i} \bar{x}_{i}$
...
| :--- | :---: | :---: | :---: | :---: |
| Source | $d f$ <br> degrees of <br> Freedom | $S S$ of <br> Squares | Mean <br> Squere | $F$-Stat |
...
Hypothesis Test
$H_{0}: \mu_{1}=\mu_{2} \ldots .=\mu_{k} \quad \xrightarrow{\text { all }}$ means are equal)
tai. Al least one $\mu_{j}, j=1, \ldots, k$ fat least one mean different) is different Lat least one treatment has a different effect
...
Conclusions
![](https://cdn.mathpix.com/cropped/2025_07_03_0e611eebd5aab7c4aee1g-05.jpg?height=737&width=1476&top_left_y=1091&top_left_x=700)
usual way
...
| consider <br> $\alpha=0.05$ | Brand 1 | Brand 2 | Brand 3 |
| :--- | :--- | :--- | :--- |
...
Sample means
Group 1 : $\bar{x}_{1}=\frac{1}{n_{1}} \sum_{j=1}^{n_{1}} x_{1 j}=\frac{1}{6}(1+3 \ldots+0)=1,33$
Group 2 : $\bar{x}_{2}=\frac{1}{n_{2}} \sum_{j=1}^{n} x_{2 j}=\frac{1}{6}(2+2 \ldots+4)=2,50$
Group 3: $\bar{x}_{3}=\frac{1}{n_{3}} \sum_{j=1}^{n_{1}} x_{3 j}=\frac{1}{6}(1+2 \ldots+4)=2.67$
Overall mean
(1)
(3)
...
Sample Variances
Group 1: SI ${ }_{1}^{2}=\frac{1}{n_{1}-1} \sum_{j=1}^{n_{1}}\left(x_{i j}-\bar{x}_{1}\right)^{2}$
...
Hypothesis Test
$H_{0}: \mu_{1}=\mu_{2} \ldots .=\mu_{k} \quad \xrightarrow{\text { all }}$ means are equal)
tHai. At least one $\mu_{j}, j=1, \ldots, k$ fat least one mean different) is different Lat least one treatment has a different effect
![](https://cdn.mathpix.com/cropped/2025_07_03_0e611eebd5aab7c4aee1g-42.jpg?height=1405&width=2748&top_left_y=0&top_left_x=1)

Hypothesis Test
$H_{0}: \mu_{1}=\mu_{2}=\mu_{3} \quad \rightarrow$ all treatments equally affective
tHai: At least one $\mu_{j}, j=1, \ldots, 3$ fat least one mean different) is different Lat least one treatment has a different effect
...
![](https://cdn.mathpix.com/cropped/2025_07_03_0e611eebd5aab7c4aee1g-43.jpg?height=944&width=1892&top_left_y=938&top_left_x=587)
2.70 (table)
...
Insufficient evidence to reject $H_{0}: \mu_{1}=\mu_{2}=\mu_{3}$ The analysis from AMOUA Suggests all 3 groups have same mean.
...
$S S T=6(1.33-2.17)^{2}+6(2.50-2.17)^{2}+6(2.67-2.17)^{2}=$ $6.387 \approx 6.39$
...
| of a surv |
| are assigned |
| the same | \$\left\{\begin{array}{l}Y_{i j}=\underbrace{\mu+T_{i}}+\varepsilon_{i j} <br>

Y_{i j}=\mu_{i}+\varepsilon_{i j},\end{array} \varepsilon_{i j} \sim N\left(0, \sigma^{2}\right)\right.\$ your mean

## Assumptions

I'Obser vutions are independent
![](https://cdn.mathpix.com/cropped/2025_07_03_0e611eebd5aab7c4aee1g-59.jpg?height=367&width=730&top_left_y=570&top_left_x=1391)
...
a. Is there sufficient evidence at the $5 \%$ significance level to infer that the bumpers differ in their reactions to low-speed collisions? b. If differences exist, which bumpers differ? Apply Fisher's LSD method with the Bonferroni adjustment.
...
![](https://cdn.mathpix.com/cropped/2025_07_03_0e611eebd5aab7c4aee1g-65.jpg?height=208&width=829&top_left_y=470&top_left_x=0)
...
Sufficient Evidence to reject $H_{0}$ and conclude at least one soup of bumpers has a different meas repair cost.
...
For each pair of groups $i$ and $j$, $i=1, \ldots, k ; \quad j=1, \ldots, k ; \quad i \neq j$
...
Fisher's is easier to calculate, has mere statistical power, homer Bunferroni maintains experiment wit error rate
...
Bonles roni
as $m p \quad d e=\left(1-\frac{\alpha}{m}\right)^{m}$ remains almust constart
...
![](https://cdn.mathpix.com/cropped/2025_07_03_0e611eebd5aab7c4aee1g-70.jpg?height=661&width=1185&top_left_y=94&top_left_x=51)
...
For the bumper example, calculate a $95 \%$ pairwise CI for the difference between Bumper (1) and Bumper (2) using LSD and Bonferroni Fisher's LSD $\quad\left(\mu_{1}-\mu_{2}\right)$
l-の $=0.98 \quad k=4 \quad$ つ $=40$ $\alpha=0.05$
$\left(\bar{x}_{1}-\bar{x}_{2}\right) \pm t_{r n-k, \alpha / 2} \cdot \sqrt{\text { MSE }\left(\frac{1}{n_{1}}+\frac{1}{n_{2}}\right)}$ from ANOUA $==\underbrace{(380-485.9) \pm t_{(36,0.025)}}_{1} \cdot \sqrt{123999\left(\frac{1}{10}+\frac{1}{10}\right)}$
...
Posthoc multiple comparisons of means : Fisher LSD $95 \%$ family-wise confidence level
\$type
...
Posthoc multiple comparisons of means : Bonferroni $95 \%$ family-wise confidence level
\$type
...
We can see that only one pair of sample means differ by more than 59.71. That is, $\left|\bar{x}_{1}-\bar{x}_{2}\right|=75.45$, and the other two differences are less than LSD. We conclude that only $\mu_{1}$ and $\mu_{2}$ differ.
If we perform the LSD procedure with the Bonferroni adjustment, the number of pairwise comparisons is 3 (calculated as $C=k(k-1) / 2=3(2) / 2$. We set $\alpha=0.05 / 3=0.0167$. Thus, $t_{\alpha / 2, n-k}=t_{0.0083,57}=-2.4682794$ and
